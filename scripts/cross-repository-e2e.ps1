...
try {
  ...
  for ($index = 0; $index -lt 12; $index++) {
    $entry = Post "/v1/context-entries" @{ templateId = $templateId; values = @{ task_clarity = $(if ($index -lt 6) { 2 } else { 4 }); start_delay = $(if ($index -lt 6) { 40 } else { 10 }) } }
    $entryIds += $entry.id
  ...
  $probeQuery = "?profileId=$([uri]::EscapeDataString($profileId))&from=2026-07-01T00:00:00.000Z&to=2026-08-01T00:00:00.000Z&timezone=Asia%2FTokyo"
  ...
  & node --experimental-strip-types --input-type=module -e "import { PcsIntegrationClient } from './apps/api/src/personalContextClient.ts'; import { assertValidPcsAnalysisSnapshotV2 } from './packages/contracts/src/pcsAnalysisSnapshotV2.ts'; import { analyzePcsAnalysisSnapshot } from './packages/self-understanding/src/pcsSnapshotAnalysis.ts'; try { const snapshot=assertValidPcsAnalysisSnapshotV2(await new PcsIntegrationClient().getAnalysisSnapshot({profileId:process.env.PCS_PROFILE_ID,from:'2026-07-01T00:00:00.000Z',to:'2026-08-01T00:00:00.000Z',timezone:'Asia/Tokyo'})); const result=analyzePcsAnalysisSnapshot(snapshot,{minimumTotalSamples:8}); if(snapshot.contractRevision !== 'pcs-analysis-snapshot-v2.1') throw new Error('revision mismatch'); if(snapshot.records.length !== 12) throw new Error('record count mismatch'); if(result.status !== 'ready' || result.hypotheses.length < 1) throw new Error('hypothesis generation mismatch'); console.log(JSON.stringify({ok:true,revision:snapshot.contractRevision,records:snapshot.records.length,status:result.status,candidates:result.hypotheses.length})); } catch (error) { console.error(error instanceof Error ? error.message : error); process.exit(1); }"
