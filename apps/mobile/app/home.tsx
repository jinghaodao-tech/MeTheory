...
export default function Home() {
  ...
  return <Screen eyebrow="METHEORY / TODAY" title="観察から、少しずつ"><Section><Card tone="teal"><CardTitle>追跡中の仮説</CardTitle><CardValue>{data?.hypothesis?.statement ?? '読み込み中'}</CardValue><Text style={styles.muted}>{data?.hypothesis?.state === 'tracking' ? 'Tracking' : ''}</Text></Card><View style={styles.row}><Card><CardTitle>評価履歴</CardTitle><CardValue>{data?.evaluations ?? 0} 件</CardValue></Card><Card><CardTitle>直近7日</CardTitle><CardValue>{data?.recentCheckins ?? 0} 観測</CardValue></Card></View><Button label="今すぐチェックイン" onPress={() => router.push('/checkin')} /><Button label="仮説候補を見る" onPress={() => router.push('/hypotheses' as never)} secondary /><Button label="Evidenceを見る" onPress={() => router.push('/evidence')} secondary /><Button label="Self Modelを見る" onPress={() => router.push('/self-model')} secondary /><Button label="設定" onPress={() => router.push('/settings')} secondary /></Section></Screen>;
}
