...
export default function Evidence() {
  ...
  const records = samples.map((sample) => ({ ...sample, payload: JSON.parse(sample.payload_json ?? '{}'), period: new Date(sample.scheduled_at).getHours() >= 6 && new Date(sample.scheduled_at).getHours() < 18 ? 'day' : 'night' }));
  return (
    <Screen eyebrow="EVIDENCE" title="Observation and Evaluation">
      <Section>
        ...
        <Card><CardTitle>Observed Facts</CardTitle>{['day', 'night'].map((period) => { const rows = records.filter((record) => record.period === period && record.included); const started = rows.filter((record) => record.payload.started === true).length; const completed = rows.filter((record) => record.payload.completed === true).length; return <Text key={period} style={{ color: colors.ink }}>{period === 'day' ? 'Day' : 'Night'}: {rows.length} observations / started {percentage(started, rows.length)} / completed {percentage(completed, rows.length)}</Text>; })}</Card>
        <Card><CardTitle>Evaluation</CardTitle><CardValue>{resultLabel(evaluation?.result)}</CardValue><Text style={{ color: colors.ink }}>Observed effect: {evaluation?.observed_effect == null ? '-' : Number(evaluation.observed_effect).toFixed(2)}</Text><Text style={{ color: colors.ink }}>Required effect: {evaluation?.required_effect == null ? '-' : Number(evaluation.required_effect).toFixed(2)}</Text><Text style={{ color: colors.muted }}>Evaluator: {evaluation?.evaluator_version ?? 'comparison-v1'} / Spec: {evaluation?.hypothesis_spec_version ?? '1'}</Text></Card>
        <Card tone="amber"><CardTitle>Data Quality</CardTitle><Text style={{ color: colors.ink }}>Missing: {evaluation?.missing_count ?? 0} / Excluded: {evaluation?.excluded_count ?? 0}</Text><Text style={{ color: colors.muted }}>{evaluation?.data_quality_json ? JSON.parse(evaluation.data_quality_json).join(', ') || 'No quality flags' : 'No evaluation yet'}</Text></Card>
        <Card><CardTitle>Evidence Records</CardTitle>{(samples.length ? samples : checkins).slice(0, 12).map((record: any) => { const payload = record.payload ?? JSON.parse(record.payload_json ?? '{}'); const hour = new Date(record.scheduled_at).getHours(); return <Text key={record.id} style={{ color: colors.ink, lineHeight: 21 }}>{new Date(record.scheduled_at).toLocaleString()} / {hour >= 6 && hour < 18 ? 'Day' : 'Night'} / {payload.activity_type ?? '-'} / started {payload.started ? 'yes' : 'no'} / completed {payload.completed ? 'yes' : 'no'} / energy {payload.energy ?? '-'} / {record.included === false ? `excluded: ${record.exclusion_reason ?? record.response_status ?? 'unanswered'}` : 'included'}</Text>; })}</Card>
