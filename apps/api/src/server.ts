...
import { generateSelfUnderstanding, type UnderstandingRecord } from "../../../packages/self-understanding/src/index.ts";
...
const server = createServer(async (request, response) => {
  ...
  try {
    ...
    if (request.method === "POST" && parts.join("/") === "v1/self-understanding/analyze") {
      const input = await body(request); const userId = optionalString(input.userId) ?? ""; if (!userExists(userId)) return json(response, 404, { error: "user_not_found" }); return json(response, 200, analyzeSelfUnderstanding(userId, input));
    }
    ...
    if (request.method === "POST" && parts.join("/") === "v1/self-understanding/reviews") {
      const input = await body(request); const userId = optionalString(input.userId) ?? ""; const rating = optionalString(input.rating) ?? ""; if (!userExists(userId)) return json(response, 404, { error: "user_not_found" }); if (!["fits", "does_not_fit", "on_hold"].includes(rating)) return json(response, 400, { error: "hypothesis_review_invalid" }); const reviewId = id("hyp_review"); const candidateId = optionalString(input.candidateId) ?? ""; const createdAt = now(); db.prepare("INSERT INTO hypothesis_reviews(id,user_id,candidate_id,rating,note,created_at) VALUES(?,?,?,?,?,?)").run(reviewId, userId, candidateId, rating, optionalString(input.note) ?? "", createdAt); if (rating === "fits") db.prepare("INSERT INTO self_model_candidates(id,user_id,candidate_id,statement,status,created_at) VALUES(?,?,?,?,?,?)").run(id("self_model_candidate"), userId, candidateId, optionalString(input.statement) ?? "", "proposed", createdAt); return json(response, 201, { id: reviewId, candidateId, rating, selfModelUpdate: rating === "fits" ? "proposed" : "none" });
    }
