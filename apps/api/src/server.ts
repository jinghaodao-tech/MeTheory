...
const server = createServer(async (request, response) => {
  ...
  try {
    ...
    if (request.method === "POST" && parts.join("/") === "v1/self-understanding/self-model-candidates/review") {
      const input = await body(request); const userId = optionalString(input.userId) ?? ""; const candidateId = optionalString(input.candidateId) ?? ""; const status = optionalString(input.status) ?? ""; if (!userExists(userId)) return json(response, 404, { error: "user_not_found" }); if (!["accepted", "rejected"].includes(status)) return json(response, 400, { error: "self_model_review_invalid" }); const candidate = db.prepare("SELECT statement FROM self_model_candidates WHERE user_id=? AND id=? AND status='proposed'").get(userId, candidateId) as { statement: string } | undefined; if (!candidate) return json(response, 404, { error: "self_model_candidate_not_found" }); const reviewedAt = now(); db.exec("BEGIN IMMEDIATE"); try { db.prepare("UPDATE self_model_candidates SET status=?,reviewed_at=? WHERE user_id=? AND id=?").run(status, reviewedAt, userId, candidateId); const beliefId = status === "accepted" ? id("belief") : null; if (beliefId) db.prepare("INSERT INTO self_beliefs(id,user_id,statement,source_kind,created_at) VALUES(?,?,?,?,?)").run(beliefId, userId, candidate.statement, "user", reviewedAt); db.exec("COMMIT"); return json(response, 200, { candidateId, status, selfBeliefId: beliefId }); } catch (error) { db.exec("ROLLBACK"); throw error; }
    }
