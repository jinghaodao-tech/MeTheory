...
const server = createServer(async (request, response) => {
  ...
  try {
    ...
    if (parts[0] === "v1" && parts[1] === "ai" && request.method === "GET") {
      ...
      if (parts.length === 3 && parts[2] === "self-model") { const selfModelAllowed = process.env.METHEORY_AI_SELF_MODEL_ALLOWED === 'true'; const rows = selfModelAllowed ? db.prepare("SELECT statement,created_at FROM self_beliefs WHERE user_id=? ORDER BY created_at DESC LIMIT 10").all(userId) as Array<Record<string, unknown>> : []; writeAiAudit({ userId, clientId, clientType, purpose, parameterIds: [], allowed: [], denied: [], count: rows.length, status: selfModelAllowed ? "allowed" : "denied" }); return json(response, 200, { items: rows, accessLevel: selfModelAllowed ? 'explicit_self_model_consent' : 'none' }); }
      ...
      if (parts.length === 5 && parts[2] === "hypotheses" && parts[4] === "evidence") { if (!db.prepare("SELECT 1 FROM hypotheses WHERE user_id=? AND id=?").get(userId, parts[3])) return json(response, 404, { error: "hypothesis_not_found" }); const rows = db.prepare("SELECT e.direction,e.rule_version,e.created_at,o.field FROM evidence_links e JOIN observations o ON o.id=e.observation_id WHERE e.hypothesis_id=? ORDER BY e.created_at DESC").all(parts[3]); writeAiAudit({ userId, clientId, clientType, purpose, parameterIds: [], allowed: [], denied: [], count: rows.length, status: "allowed" }); return json(response, 200, { items: rows }); }
