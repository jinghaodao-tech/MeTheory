  ...
  try {
    ...
    */
    ...
    if (request.method === "POST" && parts.join("/") === "v1/self-understanding/analyze-personal-context") {
      ...
      const result = analyzePcsAnalysisSnapshot(snapshot as any, { minimumTotalSamples: Number(input.minimumEntryCount ?? 8) });
      let run;
      try { run = pcsAnalysisRepository.saveRun(userId, snapshot as any, result); }
