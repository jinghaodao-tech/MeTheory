...
export class SqliteTemplateRepository {
  ...
  private insertVersion(templateId: string, versionId: string, draft: TemplateDraft, input: any, version: number, timestamp: string) {
    ...
    for (const field of draft.fields) {
      ...
      this.db.prepare("INSERT INTO entry_template_fields(id,template_version_id,field_key,label,description,input_type,value_type,required,display_order,options_json,minimum,maximum,unit,sensitivity,reason,sensitivity_level,classification_source,prohibited_secret_risk,semantic_role,semantic_role_source,semantic_role_confidence,semantic_role_confirmed,semantic_merge_allowed) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)").run(id("template_field"), versionId, field.fieldKey, field.label, field.description ?? "", field.inputType, field.valueType, field.required ? 1 : 0, field.displayOrder, JSON.stringify(field.options ?? []), field.minimum ?? null, field.maximum ?? null, field.unit ?? null, field.sensitivity, field.reason, policy.sensitivityLevel, policy.classificationSource, policy.prohibitedSecretRisk ? 1 : 0, field.semanticRole ?? null, field.semanticRoleSource ?? null, field.semanticRoleConfidence ?? null, field.semanticRoleConfirmed ? 1 : 0, field.semanticMergeAllowed ? 1 : 0);
    }
  }
