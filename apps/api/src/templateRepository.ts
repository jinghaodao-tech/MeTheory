...
export class SqliteTemplateRepository {
  ...
  private insertEntryValue(entryId: string, versionId: string, field: any, value: unknown, timestamp: string) {
    this.db.prepare("INSERT INTO entry_field_values(id,entry_id,template_version_id,template_field_id,text_value,integer_value,number_value,boolean_value,json_value,date_value,datetime_value,duration_seconds,is_missing,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)").run(...([id("entry_value"), entryId, versionId, field.id, typeof value === "string" ? value : null, typeof value === "number" && Number.isInteger(value) ? value : null, typeof value === "number" && !Number.isInteger(value) ? value : null, typeof value === "boolean" ? (value ? 1 : 0) : null, Array.isArray(value) ? JSON.stringify(value) : null, field.value_type === "date" ? value : null, field.value_type === "datetime" ? value : null, field.value_type === "duration_seconds" ? value : null, value === null ? 1 : 0, timestamp] as any[]));
  }
