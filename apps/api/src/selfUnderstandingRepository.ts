...
export class SqliteSelfUnderstandingRepository {
  ...
  analyze(userId: string, input: Record<string, unknown>) {
    ...
    const hypothesesWithBaseline = hypotheses.map((hypothesis) => {
      ...
      if (!mapping || !response) return hypothesis;
      return {
        ...hypothesis,
        baselineComparison: {
          source: "baseline_self_perception",
          itemKey: mapping.itemKey,
          response: response.response,
          recordedAt: response.recorded_at,
          direction: mapping.direction,
          message: "This is a general self-perception and is shown alongside, not averaged with, daily observations."
        }
      };
    });
