...
function withEntryMetadata(text: string, metadata: Record<string, string | boolean>) { const match = /^(---\r?\n)([\s\S]*?)(\r?\n---\r?\n?)/.exec(text); const values = new Map<string, string>(); if (match) for (const line of match[2].split(/\r?\n/)) { const separator = line.indexOf(":"); if (separator > 0) values.set(line.slice(0, separator).trim(), line.slice(separator + 1).trim()); } for (const [key, value] of Object.entries(metadata)) values.set(key, String(value)); const frontmatter = ["---", ...[...values].map(([key, value]) => `${key}: ${value}`), "---", ""].join("\n"); return `${frontmatter}${match ? text.slice(match[0].length) : text}`; }
async function offerTemplate(context: ExtensionContext, path: string, theme: string) { try { const suggestions = await templates(theme); const items = [...suggestions.map(item => ({ label: item.name, description: item.description, detail: `Existing template · ${item.field_count ?? 0} fields`, id: item.id, action: "attach" })), { label: "Continue without a template", description: "Keep this note as a normal tracked note", action: "skip" }, { label: "Generate a template draft", description: "Create a draft that must be approved before use", action: "generate" }]; const selected = await window.showQuickPick(items, { placeHolder: "Template suggestions are optional and do not block writing" }); if (!selected || selected.action === "skip") return; if (selected.action === "generate") { await cli(context, ["template", "generate", "--theme", theme, "--json"]); window.showInformationMessage("MeTheory: template draft created. Approve it before attaching it to this note."); return; } if (selected.id) { await cli(context, ["entry", "template", "attach", path, selected.id, "--json"]); window.showInformationMessage(`MeTheory: attached approved template ${selected.label}`); } } catch (error) { window.showWarningMessage(`MeTheory template processing deferred: ${error instanceof Error ? error.message : "unavailable"}`); } }
async function createNote(context: ExtensionContext, theme: string) { const normalizedTheme = theme.trim(); if (!normalizedTheme) return; const title = await window.showInputBox({ prompt: "Note title", value: normalizedTheme }); if (!title) return; const directory = join(root(), "notes", "inbox"); await mkdir(directory, { recursive: true }); const date = new Date().toISOString(); let filename = `${date.slice(0, 10)}-${safeSegment(title)}.md`; let suffix = 2; while (existsSync(join(directory, filename))) filename = `${date.slice(0, 10)}-${safeSegment(title)}-${suffix++}.md`; const path = join(directory, filename); const body = `# ${title}\n\n`; await writeFile(path, ["---", `recorded_at: ${date}`, "tracked: true", "auto_structure: true", "template_status: pending", "---", "", body].join("\n"), "utf8"); if (workspace.getConfiguration("metheory.notes").get("openAfterCreate", true)) await commands.executeCommand("vscode.open", Uri.file(path)); window.showInformationMessage(`MeTheory: created ${filename}`); void (async () => { try { await startService(context); await cli(context, ["entry", "sync", path, "--json"]); await offerTemplate(context, path, normalizedTheme); } catch (error) { window.showWarningMessage(`MeTheory saved the note; background setup is deferred: ${error instanceof Error ? error.message : "unavailable"}`); } })(); }
...
export function activate(context: ExtensionContext) {
  ...
  register("metheory.openSelfUnderstanding", async () => commands.executeCommand("workbench.view.extension.metheory"));
