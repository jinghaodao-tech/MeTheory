...
import { ActivityWatchAdapter, normalizeActivityWatchEvent } from "../packages/domain/src/activitywatch.ts";
...
test("ActivityWatch adapter is localhost-only and discards AFK and raw private fields", async () => {
  ...
  const adapter = new ActivityWatchAdapter({ fetchImpl: async (url) => new Response(url.endsWith("/api/0/buckets") ? JSON.stringify({ "aw-watcher-window": { type: "window" } }) : "[]", { status: 200, headers: { "content-type": "application/json" } }) });
...
test("baseline self-perception uses an independent version and explicit validation", () => {
  ...
  assert.equal(response.source, "ipip");
