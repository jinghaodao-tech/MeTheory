...
export function localDateInTimeZone(timestamp: string, timezone = "UTC"): string {
  const instant = new Date(timestamp);
  if (!Number.isFinite(instant.getTime())) throw new Error("activitywatch_timestamp_invalid");
  ...
  try {
    ...
    throw new Error("activitywatch_timezone_invalid");
  }
  const value = (type: Intl.DateTimeFormatPartTypes) => parts.find((part) => part.type === type)?.value;
  const year = value("year");
  const month = value("month");
  const day = value("day");
  if (!year || !month || !day) throw new Error("activitywatch_timezone_invalid");
  return `${year}-${month}-${day}`;
}

export function activityWatchObservationIdentity(bucketId: string, sourceEventId?: string, fallback?: { timestamp: string; category: ExternalObservation["category"]; projectLabel?: string; durationSeconds?: number }): string {
  const identity = sourceEventId
    ? ["activitywatch", bucketId, sourceEventId, fallback?.durationSeconds ?? ""]
    : ["activitywatch", bucketId, fallback?.timestamp ?? "", fallback?.durationSeconds ?? "", fallback?.category ?? "other", fallback?.projectLabel ?? ""];
  return createHash("sha256").update(JSON.stringify(identity)).digest("hex").slice(0, 32);
}

export function normalizeActivityWatchEvent(bucketId: string, event: ActivityWatchEvent, importedAt = new Date().toISOString(), timezone = "UTC"): ExternalObservation | null {
  ...
  const categorized = category(data);
  const projectLabel = safeProjectLabel(data);
  const observedAt = new Date(event.timestamp).toISOString();
  const sourceIdentity = activityWatchObservationIdentity(bucketId, sourceEventId, { timestamp: observedAt, durationSeconds, category: categorized, projectLabel });
  return { id: `activitywatch_${sourceIdentity}`, source: "activitywatch", sourceBucketId: bucketId, sourceEventId, sourceIdentity, observedAt, localDate: localDateInTimeZone(observedAt, timezone), ...(durationSeconds === undefined ? {} : { durationSeconds }), semanticRole: durationSeconds && durationSeconds > 60 ? "task_continuation" : "observed_behavior", category: categorized, ...(projectLabel ? { projectLabel } : {}), privacyLevel: projectLabel ? "sensitive" : "normal", importedAt };
}
...
export class ActivityWatchAdapter {
  ...
  async preview(bucketIds: string[], startAt: string, endAt: string, timezone = "UTC"): Promise<ExternalObservation[]> { const all = (await Promise.all(bucketIds.map(async (bucketId) => (await this.events(bucketId, startAt, endAt)).map((event) => normalizeActivityWatchEvent(bucketId, event, new Date().toISOString(), timezone))))).flat(); return all.filter((item): item is ExternalObservation => Boolean(item)); }
}
