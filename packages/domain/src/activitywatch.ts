...
export function activityWatchObservationIdentity(bucketId: string, sourceEventId?: string, fallback?: { timestamp: string; category: ExternalObservation["category"]; projectLabel?: string }): string {
  const identity = sourceEventId
    ? ["activitywatch", bucketId, sourceEventId]
    : ["activitywatch", bucketId, fallback?.timestamp ?? "", fallback?.category ?? "other", fallback?.projectLabel ?? ""];
...
export function normalizeActivityWatchEvent(bucketId: string, event: ActivityWatchEvent, importedAt = new Date().toISOString(), timezone = "UTC"): ExternalObservation | null {
  ...
  const sourceIdentity = activityWatchObservationIdentity(bucketId, sourceEventId, { timestamp: observedAt, category: categorized, projectLabel });
